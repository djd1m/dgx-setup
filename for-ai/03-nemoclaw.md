...
| `openai` | ключ | ⚠️ не в этом рецепте — см. ниже |
...
> **NOT VERIFIED — исправление.** Раньше `openai` и `gemini` стояли в одной строке с
> `anthropic` под пометкой «страна не поддерживается → СТОП». Для Anthropic это
> [подтверждено](https://www.anthropic.com/supported-countries); **для OpenAI и Google
> источник не проверялся** — ограничение было приписано по аналогии. **Не воспроизводить
> это утверждение** и не сообщать его человеку как факт.
>
> На рецепт это не влияет: `ollama` остаётся единственным вариантом здесь — **не потому,
> что остальные запрещены, а потому, что ему не нужны ни ключ, ни аккаунт**. Любой
> провайдер с ключом требует, чтобы ключ выдал человек (стоп-условие), и отправляет данные
> наружу. Если человек просит `openai` или `gemini` — это его решение и его ключ:
> остановиться и спросить, а не отказывать со ссылкой на непроверенное.
