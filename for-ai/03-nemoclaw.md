...
OpenShell, которая изолирует и запускает агента OpenClaw, на DGX Spark / GB10 (aarch64, DGX OS).
...
```bash
# 1. Архитектура
uname -m                                  # DGX Spark → aarch64; классический DGX → x86_64
                                          # записать результат: он нужен в Шаге 2
...
| `anthropic` | ключ | ❌ [страна не поддерживается](https://www.anthropic.com/supported-countries) → **СТОП** |
| `openai` | ключ | ❌ [страна не поддерживается](https://developers.openai.com/api/docs/supported-countries) → **СТОП** |
| `gemini` | ключ | ⚠️ не в этом рецепте — см. ниже |
| `custom` | свой OpenAI-совместимый URL | ⚠️ можно, но не в этом рецепте |
| **`ollama`** | **ничего** | ✅ **используй это** |

> **OpenAI — проверено, основание есть.** России нет в
> [списке поддерживаемых стран API](https://developers.openai.com/api/docs/supported-countries);
> список закрытый — *«If a location is not included in the list below, our API is not
> supported there»*, а доступ из неподдерживаемой страны *«may result in your account being
> blocked or suspended»*. Это политика провайдера. Обходных путей не предлагать.
>
> **Gemini — `NOT VERIFIED`.** Раньше он стоял в одной строке с `anthropic` под чужой
> пометкой; источник по Google не проверялся. **Не воспроизводить** это утверждение и не
> сообщать человеку как факт. Просит `gemini` — остановиться и спросить, а не отказывать
> со ссылкой на непроверенное.
>
> Главное для рецепта: `ollama` здесь единственный вариант **не потому, что остальные
> запрещены, а потому, что ему не нужны ни ключ, ни аккаунт**. Любой провайдер с ключом
> требует, чтобы ключ выдал человек (стоп-условие), и отправляет данные наружу.
...
### DGX Spark под чужим брендом — отдельный случай

Если `uname -m` → `aarch64`, а DMI выдаёт что-то вроде `Dell Pro Max with GB10 FCM1253` /
`Dell Inc.`, это **DGX Spark в OEM-корпусе**. Слов «DGX» и «Spark» в DMI нет, поэтому:

- шаблон `/DGX[_\s-]+Station/i` **не сработает** → `NEMOCLAW_NO_EXPRESS` не нужна;
- установщик отнесёт машину к классу `linux` (**P0 Tested**), а не к `spark`.

> **NOT VERIFIED — остановиться и предупредить человека.** Как класс `linux` выбирает
> архитектуру образов (по `uname -m` или по классу платформы) — в документации NVIDIA
> не описано. Если по классу, то arm64-машина в классе `linux` может получить x86-образы.
> **Первый `nemoclaw onboard` не запускать в фоне и не оставлять без присмотра:**
> проследить, что именно качается, и проверить архитектуру образа до того, как уйдёт
> много гигабайт. Обнаружил несоответствие — остановиться, не «пробовать всё равно».
