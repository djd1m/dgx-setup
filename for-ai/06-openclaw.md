...
Цель: установить OpenClaw на DGX Spark / GB10 (aarch64, DGX OS), направить его на локальную Ollama **без `/v1`**, привести безопасность в осознанное состояние и убедиться, что агент **выполняет** инструменты, а не печатает их вызовы текстом.
...
**У OpenClaw песочница по умолчанию выключена**, а он выполняет произвольные shell-команды. [NemoClaw](03-nemoclaw.md) существует ровно для того, чтобы запускать OpenClaw **внутри изолированной песочницы OpenShell**.
...
Выполнена инструкция [00-ollama.md](00-ollama.md).
...
🛑 **На DGX Spark этот совет буквально применять НЕЛЬЗЯ — он писался для другого железа.**
См. [00-ollama.md](00-ollama.md): узкое место GB10 — пропускная способность 273 ГБ/с, а не объём. «Крупнее» и «менее сжато» = больше байт через шину на токен = медленнее. Плотная 70B по [замерам](https://ollama.com/blog/nvidia-spark-performance) даёт **4.4 tok/s** — формально «крупнейшая, что влезла», практически непригодна для агентного цикла.

**Как выполнить требование OpenClaw, не нарушив ограничение железа:** оно про **способности** модели, а не про вес файла. Брать **MoE** — `gpt-oss:120b` силён как 120B и при этом даёт **41 tok/s**. Сильная модель без платы за скорость.
...
Здесь есть натяжение, и его надо назвать вслух: **гео-ограничения гонят к локальным моделям, а документация запрещает давать инструменты слабым моделям.** Разрешается тем, что памяти ~119 ГБ — сильная модель влезет. Но на GB10 «сильная» ≠ «крупная по весу»: см. Шаг 2.

Ожидаемый результат: выбрана **MoE-модель** уровня `gpt-oss:120b` — сильная и быстрая одновременно.
Если не так, то:
- Выбрана **плотная** модель (по замеру `<10` tok/s) → инструменты на ней не включать молча: агентный цикл на такой скорости бесполезен. Взять MoE.
- Выбрана **слабая мелкая** модель → инструменты не включать. Либо взять MoE покрупнее, либо остановиться и спросить человека.

🛑 **Не выбирать модель по размеру файла** — это ровно та ошибка, против которой написан раздел «Правило выбора» в [00-ollama.md](00-ollama.md). Плотная 32B (36.9 ГБ) даёт 9.4 tok/s, MoE 120B (65.4 ГБ) — 41 tok/s. Больше файл ≠ хуже, меньше файл ≠ лучше; решает тип весов.
...
Третьего нет. «Забыл» и «оставил по умолчанию» — это провал шага, а не нейтральный исход. Если цель — изоляция, правильный ответ вообще не здесь: [03-nemoclaw.md](03-nemoclaw.md).
...
| Цель установки — изоляция | это не та инструкция. [03-nemoclaw.md](03-nemoclaw.md) |
