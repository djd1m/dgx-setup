...
```bash
# разово (дефолт не меняется; --model пробрасывается в Claude Code):
...
# сменить модель по умолчанию (перепроверит и сохранит), потом просто claude-fm:
bash scripts/install-claude-fm.sh --model "zai-org/GLM-5.2"
```

Если разовый `--model` не примется (валидация имён Claude Code) — использовать переустановку
с `--model` (id пишется в `ANTHROPIC_MODEL` обёртки, гарантированно). id — точный из `/v1/models`.
