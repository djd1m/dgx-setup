...
**Проверено по спецификации.** Скачиваемый статический YAML с OpenAPI-спецификацией Cloud.ru сам называет себя «ограниченной двумя методами»; в объекте `paths` ровно два элемента: `GET /v1/models` и `POST /v1/chat/completions`. **`/v1/messages` отсутствует.** Официальный quickstart подтверждает с другой стороны: вызов идёт через OpenAI SDK, `client.chat.completions.create`.

> Граница вывода: доказано отсутствие в **опубликованной спецификации** на дату проверки. Недокументированный эндпоинт формально не исключён, но строить на нём рецепт нельзя. **Не пытаться «нащупать» `/v1/messages` перебором** — 404 здесь ожидаемый результат, а не повод искать обход.
...
**Проверено: если `master_key` не задан, проверки ключа нет вовсе.** Документация LiteLLM помечает `general_settings.master_key` как **OPTIONAL** — он нужен, только если оператор хочет требовать ключ во всех вызовах. В коде ветка `master_key is None` пропускает запрос независимо от наличия `api_key`; комментарий там же называет это **«No-auth dev mode»**. То есть на локальном LiteLLM без `master_key` `Authorization` можно не слать вовсе, и значение `ANTHROPIC_AUTH_TOKEN` ни на что не влияет.

⚠️ **Клиентский `Authorization` НЕ проходит насквозь к провайдеру.** Функция `clean_headers` вырезает его перед вызовом Cloud.ru; credential провайдера берётся только из конфига. **Не подставлять ключ Cloud.ru в `ANTHROPIC_AUTH_TOKEN`** в надежде, что он «дойдёт» — не дойдёт, а ключ окажется в лишнем месте.

Вывод — для стандартного режима. Включённые JWT/OAuth2 или свои auth-хуки меняют картину.
...
**Если не так:**
- **401 и переменная взята правильная** → значение `ANTHROPIC_AUTH_TOKEN` не совпадает с тем, что LiteLLM настроен принимать. Сверить настройку авторизации по [документации LiteLLM](https://docs.litellm.ai/docs/anthropic_unified/) (в официальном примере — `LITELLM_KEY`). Учесть: **если `master_key` в конфиге не задан, 401 от LiteLLM невозможен в принципе** — это «no-auth dev mode», ключ не проверяется. Получил 401 без `master_key` → 401 пришёл **не от LiteLLM, а от Cloud.ru** (то есть проблема в `CLOUD_RU_FM_API_KEY`, вернуться к Шагу 1).
...
1. **Нет ключа — любого из двух.** **Никогда не выдумывать, не генерировать и не подставлять правдоподобные учётные данные.** Это относится к обоим ключам, и подставлять один вместо другого тоже нельзя:
   ...
   - `ANTHROPIC_AUTH_TOKEN` неизвестен → посмотреть, задан ли `master_key` в конфиге LiteLLM. **Не задан** → это no-auth dev mode, любое значение (и даже отсутствие заголовка) проходит — брать плейсхолдер и идти дальше. **Задан** → значение выдаёт **человек**, не подбирать. И ни в каком случае **не подставлять сюда `CLOUD_RU_FM_API_KEY`**: `clean_headers` всё равно вырежет заголовок, а ключ окажется лишний раз в окружении.
...
4. **Личный аккаунт — проверено частично, предупредить человека.** Цена `Qwen/Qwen3-Coder-Next` на 17.07.2026 по официальному каталогу: **122 ₽ / 1 млн входных**, **244 ₽ / 1 млн генерируемых**. Регистрация физлица подтверждена (все аккаунты Cloud.ru Evolution по умолчанию типа «Физлицо», оплата с баланса допускается, нужен российский номер телефона). **NOT VERIFIED:** прямого обещания, что именно Foundation Models доступен типу «Физлицо», нет, а Cloud.ru предупреждает, что **тип аккаунта влияет на доступность сервисов**. Выяснится на Шаге 1 — не обещать человеку заранее.
