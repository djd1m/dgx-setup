...
**Цель:** подключить Claude Code к моделям Cloud.ru Foundation Models на x86_64 Ubuntu / DGX OS через прокси-переводчик LiteLLM.
...
| Размер моделей | ограничен VRAM | до 397B |
...
- [ ] **Smoke-тест вызова инструментов** — агент реально читает файлы и запускает команды:
      ...
      ```
      Готово, только если в ответе есть **и** `canary-7f3a` (сработал файловый инструмент), **и** `x86_64` (сработал запуск команд). Ответ без вызова инструментов — не зачёт: это ровно тот отказ, который описан в шаге 6 и помечен **NOT VERIFIED**.
