...
### 🛑 Права на секреты/файлы внутри контейнера (частая поломка MCP)

**Симптом:** агент не читает файл учётных данных → ошибки `AUTH` → зацикливание → агент сам
предлагает `sudo chown …`. Проверено на живом контейнере (`v2026.7.7.2`, 2026-07-19):

1. **Причина — связка «владелец + режим», а не владелец сам по себе.** `root:root` + `644`
   агент читает; не читается `root:root` **+ `600`** (нормальный режим для секрета).
   Диагноз «файл принадлежит root» — неполный, не останавливаться на нём.
2. **`restart` НЕ чинит.** chown при старте применяется к каталогу `/opt/data`, но **не
   рекурсивно** к уже лежащим файлам. Проверено: после `restart` файл остался `0:0`.
3. **`sudo` в образе ОТСУТСТВУЕТ.** Не предлагать человеку `sudo …` для выполнения ИЗНУТРИ —
   невыполнимо. Правки прав — только с хоста.
4. **`docker exec` заходит под root**, а агент работает под `hermes`. Проверка чтения через
   обычный `exec` — **ложноположительная**. Проверять только так:
   `docker compose exec -T --user hermes <svc> cat <путь>`

**Лечение (сохраняя режим 600):**

```bash
docker compose exec -T <svc> id hermes                    # узнать uid агента (= HERMES_UID)
docker compose exec -T --user root <svc> chown 1000:1000 /opt/data/<путь>/creds.json
```

🛑 **НЕ предлагать `chmod 644`** — это откроет учётные данные всему в контейнере. Менять
владельца, не режим.

**Предпочтительно вообще без файла:** если потребитель (MCP-сервер и т.п.) умеет читать
переменные окружения — класть секрет в `secrets/<человек>.env`, а не в файл в томе. Тогда нет
ни вопроса прав, ни риска «файл есть, а внутри пусто». Применять `up -d --force-recreate`.

### 🛑 `AUTH` у MCP — ДВЕ независимые причины, проверять обе

Живой случай (Cloud.ru MCP в контейнере): ошибки `AUTH` были вызваны сразу двумя причинами.
**Починив одну, не считать вопрос закрытым.**

1. **Права** — агент не читает файл (разбор выше).
2. **Пустые поля** — файл читается, но нужные ключи не заполнены. Обёртки MCP часто **требуют
   файл**, но конвертируют его в `export`-переменные для бинаря; **пустое поле → переменная
   не экспортируется молча** → бинарь падает с той же `AUTH`.

Проверять **обе** стороны:

```bash
docker compose exec -T --user hermes <svc> cat /opt/data/<путь>/credentials.json >/dev/null && echo "права OK"
docker compose exec -T --user hermes <svc> python3 -c \
  "import json;d=json.load(open('/opt/data/<путь>/credentials.json'));print('заполнено:',[k for k,v in d.items() if v])"
```

- **Чисто env-схема возможна не всегда:** при `if [ ! -f "$CRED" ]; then exit 1` файл обязателен.
  Путь обычно переопределяется переменной (Cloud.ru — `CLOUDRU_CREDENTIALS`) → файл можно держать
  вне тома, но не убрать.
- **MCP перечитывает креды только при перезапуске** → после починки `docker compose restart <svc>`,
  иначе сервер останется в старом провалившемся состоянии и лечение будет выглядеть неработающим.
- **Проверить `*.template.json`** рядом: там должны быть плейсхолдеры, а не реальные значения.
