...
Сверено с живым образом 2026-07-19: команда `["gateway","run"]`, тег `v2026.7.7.2`, entrypoint
`/init` не трогать, host-права через `HERMES_UID/GID` (не `--user`), bridge-сеть. Осталось
эмпирически: живой запуск на железе + `hermes model` в каждом контейнере (docker на VPS ещё не
ставили). Детали и команды — в шапке файла.
