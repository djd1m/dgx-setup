...
12. **Hermes в песочнице NemoClaw + Telegram** — если наткнулся на это сочетание, **остановись
    и спроси человека**.

    > **NOT VERIFIED:** у OpenShell есть **сетевая политика**. Пропускает ли она исходящий
    > трафик на `api.telegram.org` по умолчанию и чем именно он разрешается — **не выяснено**.
    > Исследование было прервано. **Не угадывай** конфиг сетевой политики.
