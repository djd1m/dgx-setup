...
5. **Установка пакетов идёт напрямую, не через туннель.** Реестры (GitHub, ghcr.io,
   downloads.claude.ai) гео не блокируют; через КЗ идёт только трафик моделей. Если реестры
   недоступны (российский DPI за Cloudflare) — установку делать из открытой сети, а не
   тащить пакеты через VLESS.
