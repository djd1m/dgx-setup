...
⚠️ **Этот совет на DGX Spark нельзя применять буквально — он писался для другого железа.**
Разбор в [00-ollama.md](00-ollama.md), суть коротко: узкое место GB10 — не объём памяти,
а её пропускная способность (273 ГБ/с). «Крупнее» и «менее сжато» означает **больше байт
через шину на каждый токен**, то есть медленнее. По
[замерам Ollama на этом железе](https://ollama.com/blog/nvidia-spark-performance) плотная
70B даёт **4.4 tok/s** — формально «самая большая, что влезла», практически непригодна.

**Как примирить с предупреждением OpenClaw.** Оно не про размер файла, а про **способности**
модели: слабая модель с инструментами опасна. На GB10 это решается не весом, а
**архитектурой**: `gpt-oss:120b` — MoE, и он одновременно крупный (120B параметров) и
быстрый (**41 tok/s**). То есть берёшь сильную модель, не платя за это скоростью.

**Правило для этой машины: бери MoE покрупнее, а не плотную покрупнее.**
...
моделям, а документация запрещает давать инструменты слабым моделям.**

Разрешается оно тем, что у тебя 119 ГБ памяти: сильная модель влезет. Но — см. Шаг 2 —
**не любая сильная модель на GB10 быстрая**. Бери **MoE**: `gpt-oss:120b` силён как
120-миллиардная модель и при этом выдаёт 41 tok/s. Плотная 70B тоже «сильная», но 4.4 tok/s
делают её непригодной для агента, который ходит в инструменты по кругу.

**То есть: не экономь на способностях модели, но выбирай их через MoE, а не через вес файла.**
