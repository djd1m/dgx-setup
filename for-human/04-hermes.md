...
### Первое сообщение: «No home channel is set»

При первом контакте бот, скорее всего, припишет к ответу:

> 📬 No home channel is set for Telegram. A home channel is where Hermes delivers cron job
> results and cross-platform messages. Type `/sethome` to make this chat your home channel,
> or ignore to skip.

**Это не ошибка, а предложение.** Домашний канал — чат, куда Hermes пишет **сам**, без твоего
вопроса: результаты **задач по расписанию** (cron) и **межплатформенные** сообщения. Без него
агент просто не знает, в какой чат такое отправлять.

Лечится одной командой прямо в чате:

```
/sethome
```

Не нужны задачи по расписанию — можно игнорировать, на обычные ответы это никак не влияет.

**Заранее в конфиге не пропишешь.** Домашний канал — это конкретный chat-id, и Hermes узнаёт его,
только когда ты набираешь `/sethome` **именно в том чате**, который делаешь домашним. Поэтому если
агентов несколько, каждый человек делает это сам, в своём чате со своим ботом.

> ⚠️ **Не путать с `home_mode` из `config.yaml`.** Тот ключ лежит в секции `terminal:` и задаёт
> домашний каталог для выполнения команд агента. К домашнему чату он отношения не имеет —
> правкой `home_mode` это сообщение не убрать.
...
> `HERMES_UID=$(id -u) HERMES_GID=$(id -g)` (не `--user`). Детали выверены по `Dockerfile`/`main-wrapper.sh`.
>
...
## Веб-поиск: какой провайдер и куда класть ключ

Отдельного мастера настраивать не нужно — **всё задаётся в `.env`**. Hermes сам выбирает
поисковый бэкенд по тому, **какой ключ у тебя есть**, и берёт первый доступный по приоритету:

| # | Бэкенд | Что положить в `.env` |
|---|---|---|
...
| 7 | DuckDuckGo | — (если в системе есть пакет `ddgs`) |

Достаточно вписать **один** ключ — например `TAVILY_API_KEY=...` — и поиск заработает через него.

Хочешь зафиксировать конкретный бэкенд, а не полагаться на приоритет:

```bash
hermes config set web.search_backend tavily
```

Явное значение всегда побеждает автовыбор.

### Три грабли

**1. У Brave переменная `BRAVE_SEARCH_API_KEY`, а не `BRAVE_API_KEY`.** В коде есть обе (короткая
используется для другого), но поиск читает длинную. Впишешь короткую — молча не подхватится.

**2. Платные бэкенды намеренно стоят выше бесплатных.** Это не случайность: в коде объяснено, что
твой явный ключ не должен перебиваться Nous-OAuth-токеном, у которого подписка может не давать
веб-поиск. Иначе отказ случился бы уже в работе — и **без отката** на другой бэкенд.

**3. Свой SearXNG — единственный вариант без ключей и без внешнего провайдера.** `SEARXNG_URL=…`
и всё: поисковые запросы не уходят третьей стороне.

### Если Hermes в контейнере

Дописал ключ в `secrets/*.env` → применяй **пересозданием**, не рестартом (см. грабли выше):

```bash
docker compose up -d --force-recreate hermes-dad
```

Проверить, что подхватилось, не печатая ключ:

```bash
docker compose exec -T hermes-dad sh -c \
  'case "${TAVILY_API_KEY:-}" in "") echo нет;; *) echo задан;; esac'
```
