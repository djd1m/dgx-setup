...
**Это происходит в России регулярно** — подтверждено на живой установке. Причина не в
Hermes: до `api.telegram.org` не доходит трафик от провайдера. Сам Telegram Россию
обслуживает — это ровно тот же случай, что с `ollama pull` и Cloudflare.

#### Сначала попробуй простое

```bash
TELEGRAM_FALLBACK_IPS=149.154.167.220
```

Одна строка в `~/.hermes/.env`. Само наличие такой настройки в документации намекает, что
до `api.telegram.org` **по имени** доходят не все. Сработает — на этом и остановись.

#### Не помогло — нужен SOCKS5
...
> **NOT VERIFIED:** доступен ли `api.telegram.org` из России напрямую — зависит от провайдера
> и момента. Проверяй у себя.
