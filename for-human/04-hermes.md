...
> ⚠️ Это **скелет, а не гарантированно рабочий файл**: готового multi-container `compose` в репе
> Hermes нет, часть деталей (запускает ли образ gateway по умолчанию, точный тег, хватает ли `.env`
> для модели) помечена в файле как **NOT VERIFIED** — там же команды, чтобы свериться с живым образом.
