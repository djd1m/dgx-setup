...
### 🛑 Секреты и файлы внутри контейнера: права — самая частая поломка

**Симптом:** агент (обычно через MCP-сервер) не может прочитать файл с учётными данными, сыплет
ошибками `AUTH`, крутится по кругу и в итоге сам предлагает `sudo chown …`.

Три факта, проверенных на живом контейнере, объясняют всё:

**1. Дело не во владельце, а в связке «владелец + режим».** Файл `root:root` с правами `644`
агент прочитает спокойно. Не читается именно `root:root` **+ `600`** — а `600` это как раз
правильная практика для секрета. Поэтому диагноз «файл принадлежит root» сам по себе неполный.

**2. Перезапуск это НЕ чинит.** При старте контейнер выставляет владельца на сам каталог
`/opt/data`, но **не проходит рекурсивно по уже лежащим внутри файлам**. Проверено: после
`restart` файл остался `root:root`. Ждать, что «перезапущу и само починится», бесполезно.

**3. `sudo` в образе отсутствует.** Если агент предлагает `sudo chown …` — изнутри он это
выполнить не сможет. Правки прав делаются **с хоста**.

> ⚠️ **Ловушка диагностики:** `docker exec` заходит в контейнер **под root**, а сам агент работает
> под пользователем `hermes`. Поэтому проверка «я зашёл, файл читается» — ложноположительная.
> Проверять надо от лица агента:
> ```bash
> docker compose exec -T --user hermes hermes-dad cat /opt/data/путь/creds.json
> ```

**Правильное лечение — сменить владельца, СОХРАНИВ режим `600`:**

```bash
# узнать uid агента (это твой HERMES_UID)
docker compose exec -T hermes-dad id hermes
# сменить владельца файла
docker compose exec -T --user root hermes-dad chown 1000:1000 /opt/data/путь/creds.json
```

> 🛑 **Не «чини» это через `chmod 644`.** Так файл с учётными данными станет читаемым для всего,
> что работает в контейнере. Меняй **владельца**, а не режим.

### Лучше вообще без файла: секреты через env

Файл с секретом внутри тома — источник сразу двух проблем: права и «файл есть, а внутри пусто».
Если MCP-сервер (или что угодно ещё) умеет читать **переменные окружения** — клади учётные данные
в `secrets/<человек>.env`, а не в файл внутри тома:

- вопрос владельца и прав не возникает вообще;
- секрет не оседает в томе, который переживает пересоздание контейнера;
- применяется штатно: `docker compose up -d --force-recreate`.

Файл оставляй только там, где иначе никак (формат требует именно файла).

### Симптом один (`AUTH`), а причин две

Разобранный живой случай: MCP-сервер [Cloud.ru](02-claude-code-cloudru.md) в контейнере.
Агент сыпал ошибками `AUTH` — и это оказались **две независимые причины сразу**:

1. **Права** — файл учётных данных недоступен агенту (разбор выше).
2. **Пустые поля** — файл читается, но нужные ключи в нём не заполнены.

Почему второе легко пропустить: обёртки MCP часто **требуют файл**, но внутри конвертируют его
в переменные окружения и уже их передают бинарю. И если поле в JSON пустое — **переменная просто
не экспортируется, молча**. Бинарь не получает её и падает с той же `AUTH`, что и при отказе
в правах. Починил права, обрадовался — а оно всё равно не работает.

**Проверяй обе стороны:**

```bash
# 1) читает ли файл САМ агент (а не root — см. ловушку выше)
docker compose exec -T --user hermes hermes-dad \
  cat /opt/data/<путь>/credentials.json >/dev/null && echo "права OK"

# 2) какие поля реально заполнены — только ИМЕНА, без значений
docker compose exec -T --user hermes hermes-dad python3 -c \
  "import json;d=json.load(open('/opt/data/<путь>/credentials.json'));print('заполнено:',[k for k,v in d.items() if v])"
```

**«Перевести на env» получается не всегда.** Если в обёртке есть `if [ ! -f "$CRED" ]; then exit 1`
— файл обязателен, и чисто env-схема не заработает. Обычно путь к файлу переопределяется отдельной
переменной (в случае Cloud.ru — `CLOUDRU_CREDENTIALS`): это позволяет держать файл **вне тома**,
но не отменяет его существования.

> ⚠️ **MCP-сервер перечитывает учётные данные только при перезапуске.** Починил права или заполнил
> поля — сделай `docker compose restart <сервис>`, иначе он продолжит работать со старым
> провалившимся состоянием, и ты решишь, что лечение не помогло.

> ⚠️ **Проверь файлы-шаблоны.** Если рядом лежит `credentials.template.json`, убедись, что в нём
> плейсхолдеры, а не реальные значения: шаблоны имеют обыкновение уезжать в git.
