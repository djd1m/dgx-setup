...
| `openai` | ключ | ❌ [страна не поддерживается](https://developers.openai.com/api/docs/supported-countries) |
| `gemini` | ключ | ⚠️ см. оговорку ниже |
| `custom` | свой OpenAI-совместимый URL | ⚠️ можно |
| **`ollama`** | **ничего** | ✅ **это твой вариант** |

> **Про OpenAI — проверено.** России нет в
> [списке поддерживаемых стран API](https://developers.openai.com/api/docs/supported-countries),
> и список **закрытый**: *«If a location is not included in the list below, our API is not
> supported there»*. Доступ из неподдерживаемой страны, дословно, *«may result in your
> account being blocked or suspended»*.
>
> **Про Gemini — источник я не проверял.** Раньше `gemini` стоял в одной строке с
> `anthropic` под чужой пометкой; это была ошибка, и я её не повторяю: `NOT VERIFIED`.
>
> На выбор это всё равно не влияет, и вот что важнее любых списков: `ollama` выигрывает
> потому, что **ему не нужны ни ключ, ни аккаунт, ни чьё-либо разрешение**, а модель
> работает на твоём железе. Этот довод не зависит от содержания страновых списков — что бы
> в них ни оказалось.
