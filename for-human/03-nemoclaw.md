...
## Шаг 2. Если у тебя DGX Station A100 — обязательно прочитай

Установщик определяет платформу по DMI-строке материнской платы и возвращает `spark`,
`station`, `jetson` или общий `linux`.
...
Обычный DGX-сервер с A100/H100 попадает в класс `linux` — это самый обкатанный путь, всё хорошо.

⚠️ **Но DGX Station A100 — машина x86_64**, и её DMI-строка совпадёт с шаблоном `/DGX[_\s-]+Station/i`.
Тогда установщик решит, что ты на arm64-Station, предложит «экспресс-путь» и потащит рецепт
`nemotron-3-ultra-550b-a55b` размером примерно **352 ГБ**, чей образ на Station резолвится
в **arm64-манифест**. То есть огромная закачка, которая на x86_64 не заработает.
...
| `anthropic` / `openai` | ключ | ❌ страна не поддерживается |
| `gemini` | ключ | ❌ то же |
