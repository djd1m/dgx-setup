...
### Свежие Qwen и Kimi: что реально влезает

Два правила, по которым это считается (оба следуют из железа выше):
- **влезает** ⇔ вес в **Q4** (≈ млрд·параметров × 0.5 ГБ) плюс контекст **меньше ~119 ГБ**;
- **скорость** определяется **активными** параметрами ÷ 273 ГБ/с, а не общим размером.

**Qwen — линейка удачная, много MoE-вариантов помещается:**

| Модель | Всего/активных | Тип | Q4 | Влезает? | Скорость |
|---|---|---|---|---|---|
| [`qwen3.6:35b-a3b`](https://ollama.com/library/qwen3.6/tags) ([карточка](https://huggingface.co/Qwen/Qwen3.6-35B-A3B)) | 35B/3B | MoE | ~24 ГБ | ✅ (и Q8 ~39) | 🚀 быстро |
| [`qwen3.5:122b-a10b`](https://ollama.com/library/qwen3.5/tags) | 122B/10B | MoE | ~81 ГБ | ✅ **только Q4** | ~27 tok/s |
| [`qwen3-coder:30b`](https://ollama.com/library/qwen3-coder/tags) ([карточка](https://huggingface.co/Qwen/Qwen3-Coder-30B-A3B-Instruct)) | 30B/3.3B | MoE | ~19 ГБ | ✅ | 🚀 быстро |
| [`qwen3-coder-next`](https://ollama.com/library/qwen3-coder-next/tags) | ~80B/3B | MoE | ~52 ГБ | ✅ | 🚀 быстро |
| [`qwen3.6:27b`](https://ollama.com/library/qwen3.6/tags) ([карточка](https://huggingface.co/Qwen/Qwen3.6-27B)) | 27B/27B | плотн. | ~17 ГБ | ✅ | ~16 tok/s |
| `qwen3-235b-a22b` | 235B/22B | MoE | ~142 ГБ | ❌ >119 | — |
| [`qwen3-coder:480b-a35b`](https://ollama.com/library/qwen3-coder/tags) | 480B/35B | MoE | ~290 ГБ | ❌ ~2.4× | — |

Мелкие плотные Qwen3 (`32b/14b/8b/…`) и `qwen3:30b-a3b` влезают тривиально.

**Kimi — флагманы локально НЕ идут.** Вся линейка K2 ([Kimi-K2.6](https://huggingface.co/moonshotai/Kimi-K2.6),
[K2.5](https://ollama.com/library/kimi-k2.5/tags), [K2.7-Code](https://ollama.com/library/kimi-k2.7-code)) —
это MoE на **~1 триллион параметров**: даже Q4 это ~500–600 ГБ, минимальный существующий
билд ([Unsloth 1.8-bit](https://huggingface.co/unsloth/Kimi-K2-Instruct-GGUF)) ~240–340 ГБ.
Это в **3–5 раз** больше 119 ГБ — не помещается ни при каком вменяемом кванте, и на Ollama
они доступны только как `:cloud` (без скачиваемых весов) или через
[NVIDIA NIM](https://build.nvidia.com/moonshotai/kimi-k2.6/modelcard). **Вывод: флагманский Kimi
требует удалённого эндпоинта** (ровно то, что делается в [02-claude-code-cloudru.md](02-claude-code-cloudru.md)).

Локально из семейства Moonshot влезают только **лёгкие** MoE — [`Kimi-VL-A3B`](https://ollama.com/richardyoung/kimi-vl-a3b-thinking)
(16B/3B, Q4 ~10 ГБ) и его текстовая база Moonlight-16B-A3B (~8 ГБ). Но это отдельный VL/лёгкий
класс, а **не** K2.

> **NOT VERIFIED (важно):** релизы **Qwen3.5 / Qwen3.6** вышли фев–апр 2026, **после** порога
> знаний. Существование, теги Ollama и размеры квантов взяты из найденных источников
> ([ollama.com/library](https://ollama.com/library/qwen3.6/tags), [QwenLM](https://github.com/QwenLM/Qwen3.6),
> HuggingFace), независимо не подтверждены. Вывод «влезает» устойчив (даже bf16-билды помещаются),
> но **точные теги и размеры проверяй сам:** `ollama pull <тег> && ollama show <тег>`. Особенно
> `qwen3.5:122b-a10b` — идентичность модели подтверждена только листингом Ollama.
>
> И помни про [баг конкурентности qwen3.5 в Ollama (#14621)](https://github.com/ollama/ollama/issues/14621):
> одиночные запросы ок, параллельные деградируют — если моделью пользуются несколько агентов сразу,
> бери что-то попроще и общее (см. [11-multi-agent-host.md](11-multi-agent-host.md)).

### Какую модель под какую задачу — и что с русским

Размер и «влезает» — это ещё не выбор. Под конкретную роль:

| Модель | Типовой сценарий | Русский |
|---|---|---|
| [`gpt-oss:120b`](https://ollama.com/library/gpt-oss) | главный мозг-оркестратор + рассуждения (41 tok/s замерено) | ❌ не подтверждён (нет в [MMMLU](https://arxiv.org/html/2508.10925v1)) |
| [`gpt-oss:20b`](https://ollama.com/library/gpt-oss) | быстрый дешёвый ход, роутинг, классификация (58 tok/s) | ❌ не подтверждён |
| [`qwen3-coder:30b`](https://huggingface.co/Qwen/Qwen3-Coder-30B-A3B-Instruct) | агентное кодирование, многофайловые правки | ⚠️ «мультиязычность» = **языки программирования**, не русский диалог |
| `qwen3:30b-a3b-thinking` | рассуждения/планы на скорости MoE | ✅ подтверждён ([блог Qwen3](https://qwenlm.github.io/blog/qwen3/)) |
| `qwen3:30b-a3b-instruct` | **русский диалог** + tool-calling + длинный контекст | ✅ подтверждён поимённо |
| `qwen3-vl:30b` | vision: документы, скриншоты, GUI | частично |

**Для общения на русском бери обычную `Qwen3-Instruct`, а не coder/gpt-oss.** Русский у Qwen
назван **поимённо** в списке 119 языков ([блог Qwen3](https://qwenlm.github.io/blog/qwen3/),
[technical report](https://arxiv.org/abs/2505.09388)); даже маленькая
[Qwen3-4B](https://huggingface.co/Qwen/Qwen3-4B) наследует это покрытие. А вот у **coder-моделей**
и **gpt-oss** русский официально **не подтверждён** — для кода отлично, для беседы на русском может
быть заметно слабее. Полный разбор облачных моделей и русского — в
[12-cloud-brain-routing.md](12-cloud-brain-routing.md).

> **NOT VERIFIED:** независимых русскоязычных бенчмарков
> ([MERA](https://mera.a-ai.ru/ru/text/leaderboard), ru_llm_arena) по флагманам 2026 нет —
> есть только по предшественникам. Качество русского финально проверяй **живым пробником**, а не
> по документации.

### 🛠 Tool-calling в Ollama — баги, которые тихо ломают агентов

Отдельный класс проблем, который **не виден по карточке модели**: даже сильная модель может
корректно **сгенерировать** вызов инструмента, но в **Ollama** сломается на подаче результата
обратно. Это баги **шаблона/парсера рантайма**, а не весов. Симптом, на который жалуются
пользователи (и который ловили на этом стеке): *модель делает вызов, получает `role:"tool"`
ответ — и молчит, цикл рвётся.*

Это **семейство** причин, почти все — в OpenAI-совместимом слое Ollama (`/v1`), до шаблона:

| Причина «пустого ответа» | Обход | Issue |
|---|---|---|
| assistant с `content:""` + `tool_calls` в истории | слать **`content:null`**, не `""` | [#14181](https://github.com/ollama/ollama/issues/14181) |
| assistant с `content` **и** `tool_calls` разом (только на `/v1`) | нативный **`/api/chat`** | [#9802](https://github.com/ollama/ollama/issues/9802) |
| thinking + tools → вызов уходит внутрь `<think>` | **`think=false`** с инструментами | [#10976](https://github.com/ollama/ollama/issues/10976) |
| молчаливый сброс при парс-фейле (`content:""`, токены есть) | tripwire: пусто + токены → retry | [#17274](https://github.com/ollama/ollama/issues/17274) |

Затронуты **почти все версии вплоть до v0.32.x**, фиксы не смёржены. Правка `TEMPLATE` в Modelfile
эти случаи **не лечит** — они в слое *до* шаблона. Самый универсальный обход: **уйти с
`/v1/chat/completions` на нативный `/api/chat`**, слать `content:null` и `think=false` с
инструментами, плюс tripwire на «пусто + ненулевые токены».

**Насколько надёжен tool-calling по моделям в Ollama:**

| Модель | Tool-calling в Ollama | Оговорка |
|---|---|---|
| `llama3.3` | ✅ безопасный дефолт | открытых багов не найдено; но русского у неё нет |
| `qwen3.5:35b-a3b` | ✅ **только на Ollama ≥ v0.30** | на v0.17.x был сломан ([#14605](https://github.com/ollama/ollama/pull/14605) смёржен) |
| `gpt-oss:120b` | ✅ при стандартных именах тулов | harmony-баг пофикшен ([#11759](https://github.com/ollama/ollama/pull/11759)) |
| `qwen3-coder:30b` | ⚠️ только с воркэраундом | XML-парсер хрупок: `content:null`, single-tool, короткий контекст ([#16686](https://github.com/ollama/ollama/issues/16686)) |
| `qwen3:30b-a3b` (MoE thinking) | ⚠️ обязательно `think=false` | [#10976](https://github.com/ollama/ollama/issues/10976) |
| `gpt-oss:20b` | ❌ избегать с произвольными именами тулов | [#11991](https://github.com/ollama/ollama/issues/11991) |
| `qwen3.6:27b` | ❌ дрейф формата → HTTP 500 в длинных диалогах | [#16383](https://github.com/ollama/ollama/issues/16383) |
| `qwen3.6:35b-a3b`, `qwen3-coder-next` | ❓ данных нет — **бенчмаркать** | — |

> 🔥 **Железное правило:** по любой локальной модели **смоук-тесть multi-turn tool-loop** (вызов →
> `role:"tool"` → продолжение) **до** того, как ей доверять. «Сгенерировала вызов» ≠ «переварит
> результат обратно». И на GB10 держи `OLLAMA_NUM_PARALLEL=1` —
> [#14621](https://github.com/ollama/ollama/issues/14621) роняет qwen3.5-арх на конкурентных запросах.
