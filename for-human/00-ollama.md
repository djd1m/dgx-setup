...
Совет `q8_0` («бери менее сжатую, раз память позволяет») **на GB10 разворачивается
в обратную сторону**: менее сжатая модель = больше байт через шину = медленнее. Память
позволяет, а шина — нет.
